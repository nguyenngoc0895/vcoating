<div class="block-element block-search-element search-style3 <?php echo esc_attr($el_class)?>">
    <?php if(!empty($title)):?>
        <h3 class="title18 font-bold text-uppercase"><?php echo esc_html($title)?></h3>
    <?php endif?>
    <div class="click_search"><span><i class="fa fa-search" aria-hidden="true"></i></span></div>
    <div class="on-search">
    <form class="search-form an <?php echo esc_attr($el_class)?> live-search-<?php echo esc_attr($live_search)?>" action="<?php echo esc_url( home_url( '/' ) ); ?>">
        ...
        <?php endif;?>
        <input name="s" onblur="if (this.value=='') this.value = this.defaultValue" onfocus="if (this.value==this.defaultValue) this.value = ''" value="<?php echo esc_attr('Nhập từ khóa tìm kiếm');?>" type="text">
