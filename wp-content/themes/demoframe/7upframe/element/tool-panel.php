...
if(!function_exists('s7upf_vc_tool_panel'))
{
    function s7upf_vc_tool_panel($attr)
    {
        ...
        $html .=    '<div class="widget-indexdm" id="widget_indexdm">
                        <a href="" class="dm-open dm-button bg-color" data-title="'.esc_attr__("Close","hama").'" data-title-close="'.esc_attr__("Demos","hama").'"><i class="fa fa-long-arrow-left"></i><i class="fa fa-long-arrow-right"></i></a>
                        <a href="'.esc_url($sp_link).'" target="_blank" class="dm-button bg-color dm-support" data-title="'.esc_attr__("Support","hama").'" data-title-close="'.esc_attr__("Support","hama").'"><i class="fa fa-support"></i></a>
                        <a href="'.esc_url($doc_link).'" target="_blank" class="dm-button bg-color dm-guide" data-title="'.esc_attr__("Guide","hama").'" data-title-close="'.esc_attr__("Guide","hama").'"><i class="fa fa-lightbulb-o"></i></a>
                        <div class="widget-indexdm-inner">
                            <div class="dm-header">
                                ...
                                <div class="header-description">
                                    <h2>'.$title.'</h2>
                                    <h4><span class="color2">'.esc_html__("Choose Your Demo","hama").'</span></h4>
                                </div>                                
                                <div class="header-button">
                                    <a target="_blank" class="shop-button1" href="'.esc_url($buy_link).'" title="'.esc_attr__("Buy Now","hama").'">'.esc_html__("Buy Now","hama").'</a>                  
                                </div>
                            </div>';
        if($colors || $colors2){
            // Main color
            $main_color = s7upf_get_value_by_id('main_color');
            if(empty($main_color)) $main_color = '#fab502';
            ...
            $html .=            '<div class="dm-content-color clearfix">
                                    <h4><span class="color2 text-uppercase title12 font-bold">'.esc_html__("Choose main color","hama").'</span></h4>
            ...
            if(isset($colors[0]) && !empty($colors[0])){
                ...
                $html .=    '<div class="dm-content-color-inner">
                                <a class="dm-color active" href="#" title="'.esc_attr__("Default","hama").'" style="background:'.esc_attr($main_color).'" data-color="'.esc_attr($main_color).'" data-rgb="'.esc_attr($rgb_df).'"></a>';
                foreach ($colors as $key => $value){
                    $value = array_merge($default_val,$value);
                    list($r, $g, $b) = sscanf($value['color'], "#%02x%02x%02x");
                    $rgb = $r.','.$g.','.$b;
                    $html .=    '<a class="dm-color" href="#" title="'.esc_attr($value['title']).'" data-color="'.esc_attr($value['color']).'" data-rgb="'.esc_attr($rgb).'" style="background:'.esc_attr($value['color']).'"></a>';
                }
                $html .=    '</div>';
            }
            ...
            if(isset($colors2[0]) && !empty($colors2[0])){
                ...
                $html .=    '<div class="dm-content-color-inner">
                                <h4><span class="color2 text-uppercase title12 font-bold">'.esc_html__("Choose main color 2","hama").'</span></h4>
                                <a class="dm-color active" href="#" title="'.esc_attr__("Default","hama").'" style="background:'.esc_attr($main_color2).'" data-color2="'.esc_attr($main_color2).'" data-rgb2="'.esc_attr($rgb2_df).'"></a>';
                foreach ($colors2 as $key => $value){
                    $value = array_merge($default_val,$value);
                    list($r, $g, $b) = sscanf($value['color'], "#%02x%02x%02x");
                    $rgb = $r.','.$g.','.$b;
                    $html .=    '<a class="dm-color" href="#" title="'.esc_attr($value['title']).'" data-color2="'.esc_attr($value['color']).'" data-rgb2="'.esc_attr($rgb).'" style="background:'.esc_attr($value['color']).'"></a>';
                }
                $html .=    '</div>';
            }
            $html .=            '</div>';
...
vc_map( array(
    "name"      => esc_html__("Tool Panel", 'hama'),
    ...
    "params"    => array(
        ...
        array(
            "type" => "textfield",
            "heading" => esc_html__("Support link",'hama'),
            "param_name" => "sp_link",
        ),
        array(
            "type" => "textfield",
            "heading" => esc_html__("Documentation",'hama'),
            "param_name" => "doc_link",
        ),
        array(
            "type" => "textfield",
            "heading" => esc_html__("Buy Link",'hama'),
            "param_name" => "buy_link",
        ),
        array(
            "type" => "attach_image",
        ...
        array(
            "type" => "param_group",
            "heading" => esc_html__("Demo colors",'hama'),
            "param_name" => "colors",
            "params"    => array(
                array(
                    "type"          => "textfield",
        ...
        array(
            "type" => "param_group",
            "heading" => esc_html__("Demo colors 2",'hama'),
            "param_name" => "colors2",
            "params"    => array(
                array(
                    "type"          => "textfield",
        ...
        array(
            "type" => "param_group",
            "heading" => esc_html__("Demo List",'hama'),
            "param_name" => "demos",
            "params"    => array(
                array(
                    "type"          => "textfield",
                ...
                array(
                    "type" => "attach_image",
                    "heading" => esc_html__("Image item",'hama'),
                    "param_name" => "image_item",
                ),
                array(
                    "type"          => "textfield",
                    "heading"       => esc_html__("Image preview Link",'hama'),
